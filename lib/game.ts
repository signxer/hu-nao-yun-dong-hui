...
export function activateAbility(state: GameState, racerId: string, action: AbilityActionId, targetId?: string): GameState {
  ...
  if (action === 'cheerleader') { const last = Math.min(...activeRacers(state).map((item) => item.position)); racers.filter((item) => item.finished === null && !item.eliminated && item.position === last).forEach((item) => { item.position = Math.min(29, item.position + 2); }); racer.position = Math.min(29, racer.position + 1); lines.push(`${racerByName(racer.name).zhName} 为最后一名加油：最后一名移动 2 格，自己移动 1 格。`); }
  if (action === 'duelist' && target) { const ownRoll = Math.floor(Math.random() * 6) + 1; const targetRoll = Math.floor(Math.random() * 6) + 1; const winner = ownRoll >= targetRoll ? racer : target; winner.position = Math.min(29, winner.position + 2); lines.push(`决斗！${racerByName(racer.name).zhName} 掷 ${ownRoll}，${racerByName(target.name).zhName} 掷 ${targetRoll}；${racerByName(winner.name).zhName} 移动 2 格。`); }
