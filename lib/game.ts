...
export function activateAbility(input: GameState, racerId: string, action: AbilityActionId, targetId?: string): GameState {
  ...
  if (action === 'third-wheel' && target) { const from = racer.position; racer.position = target.position; state.lastMovePath = from === racer.position ? undefined : [from, racer.position]; state.pendingRoll = null; state.currentRacerId = racer.id; lines.push(`${racerByName(racer.name).zhName} 使用电灯泡，从 ${from} 传送到双人格 ${racer.position}，仍可进行主移动。`); }
