...
export interface GameState { mode: GameMode; locale: Locale; phase: 'home' | 'draft' | 'race-select' | 'race' | 'result' | 'game-over' | 'room'; board: BoardName; raceNumber: number; players: PlayerState[]; racers: RacerState[]; currentPlayer: number; finishers: string[]; raceSelections: Record<string, RacerName[]>; raceSelectPlayer: number; log: string[]; roomCode?: string; connected?: number; }
